...
DROP FUNCTION IF EXISTS get_word_by_id(INTEGER);

-- Créer ou remplacer la fonction
CREATE OR REPLACE FUNCTION get_word_by_id(id_searched INTEGER)
RETURNS TABLE (
  id BIGINT,
  name VARCHAR,
  definition TEXT,
...
BEGIN
  ...
  FROM
    word w
  WHERE w.id = id_searched;
END;
$$ LANGUAGE plpgsql;

-- Appel de la fonction
SELECT * FROM get_word_by_id(10);
