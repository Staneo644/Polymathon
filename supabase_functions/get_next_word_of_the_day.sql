...
DROP FUNCTION IF EXISTS get_next_word_of_the_day(INTEGER);

-- Créer ou remplacer la fonction
CREATE OR REPLACE FUNCTION get_next_word_of_the_day(limit_count INTEGER)
RETURNS TABLE (
  id BIGINT,
  name VARCHAR,
  definition TEXT,
...
BEGIN
  ...
  FROM
    word w
  WHERE w.validated = TRUE          -- Assurer que le mot est validé
  ORDER BY w.last_day_word ASC
  LIMIT limit_count;
END;
$$ LANGUAGE plpgsql;

-- Appel de la fonction
SELECT * FROM get_next_word_of_the_day(3);
