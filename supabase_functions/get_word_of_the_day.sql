...
DROP FUNCTION IF EXISTS get_word_of_the_day();
...
BEGIN
  ...
  FROM
    word w
  WHERE
    w.last_day_word = CURRENT_DATE  -- Comparer à la date actuelle
    AND w.validated = TRUE;          -- Assurer que le mot est validé
END;
$$ LANGUAGE plpgsql;

-- Appel de la fonction
SELECT * FROM get_word_of_the_day();
