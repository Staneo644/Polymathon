...
DROP FUNCTION IF EXISTS get_popular_words(INTEGER);

-- Créer ou remplacer la fonction
CREATE OR REPLACE FUNCTION get_popular_words(limit_count INTEGER)
RETURNS TABLE (
  id BIGINT,
  name VARCHAR,
  definition TEXT,
...
BEGIN
  ...
  ORDER BY
    (subquery.likes - subquery.dislikes) DESC
  LIMIT
    limit_count;
END;
$$ LANGUAGE plpgsql;
...
SELECT * FROM get_popular_words(10);
