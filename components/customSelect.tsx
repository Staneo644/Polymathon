...
export const CustomSelect: FC<{
  setCurrent: (option: number) => void;
  themes: ThemeRow[];
  current?: number;
}> = ({ setCurrent, themes, current }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedParentOptionId, setSelectedParentOptionId] =
    useState<number>();
  ...
  return (
    <div className="custom-select-container">
      <button
        onClick={toggleOpen}
        className="bg-gray-100 p-2 rounded-md  w-60 flex justify-between"
      >
      ...
      {isOpen && (
        <ul className="custom-select-options absolute z-10 bg-gray-100 rounded-md cursor-pointer">
          {parentThemes &&
            parentThemes.map((option, index) =>
              hasChildren(themes ?? [], option.id) ? (
                <>
                  <li
                    key={option.id}
                    onClick={() => {
                      selectedParentOptionId == option.id
                        ? setSelectedParentOptionId(0)
                        : setSelectedParentOptionId(option.id);
                    }}
                  ...
                  {option.id === selectedParentOptionId && (
                    <ul className="custom-select-options absolute translate-x-32 -translate-y-10 z-10 bg-white rounded-md shadow-xl cursor-pointer">
                      {getChildrenThemes(themes ?? [], option.id).map(
                        (option, index) => (
                          <>
                            <li
                              key={option.id}
                            ...
                            {index !==
                              getChildrenThemes(themes ?? [], option.id)
                                .length -
                                1 && <hr />}
                          </>
                        ),
