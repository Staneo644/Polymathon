...
export default function MoreWorld() {
  ...
  useEffect(() => {
    if (debouncedValue) {
      fetch("http://localhost:3000/api/word/search?text=" + debouncedValue, {
        method: "GET",
      })
        .then((response) => {
          if (response.ok) {
  ...
  return (
    <>
      <div className="flex items-center w-full h-1/3 flex-col">
        <p className="text-[var(--yellow)] mt-10 font-bold">
          Entrez le mot à modifier ou ajouter
        </p>
      ...
      <div className="flex items-center w-full b-4 flex-col">
        <YellowButton
          onClick={() => {
            router.push("/plus-de-mots/ajouter-un-mot?word=" + inputValue);
          }}
          inactive={inputValue.length == 0}
          className={"font-bold py-2 px-4 rounded-full mt-4"}
        >
          Ajouter
        </YellowButton>
        {isAdmin && (
          <YellowButton
            onClick={() => {
              router.push("/plus-de-mots/valider-un-mot");
            }}
            className={"font-bold py-2 px-4 rounded-full mt-4"}
          >
          ...
          {listSearchedWord.map((word) => {
            return (
              <button
                key={word.id}
                onClick={() => {
                  router.push("/plus-de-mots/modifier-un-mot?id=" + word.id);
                }}
                className={`text-gray-900 font-bold px-4 rounded-full flex items-center w-1/2 my-2 justify-between`}
                style={{
              ...
              >
                {word.name}{" "}
                <div className="h-12 w-1 bg-black mx-4 opacity-50" /> modifier
              </button>
            );
          })}
        </div>
