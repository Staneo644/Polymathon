...
export default function validateWords() {
    ...
    useEffect(() => {
        fetch(`http://localhost:3000/api/word/unvalidated?${new URLSearchParams({ limit: "20" })}`, {method: 'GET'}).then((response) => {
        if (response.ok) {
        ...
        fetch(`http://localhost:3000/api/theme`, {method: 'GET'}).then((response) => {
        if (response.ok) {
            response.json().then((data) => {
            setThemes(data.data);
            console.log(data);
    ...
    useEffect(() => {
        if (words.length > 0)
        fetch(`http://localhost:3000/api/word/name?${new URLSearchParams({ name: words[index].name })}`, {method: 'GET'}).then((response) => {
        if (response.ok) {
            response.json().then((data) => {
                if (data.data.data && data.data.data.length > 0) {
                    ...
                    if (findTheme(data.data.data[0].theme, themes)?.name != words[index].theme) {
    ...
    return (
        <>
            <h1 className="absolute l-0">{isWordAdded ? "Mot ajouté" : "Mot modifié"}</h1>
